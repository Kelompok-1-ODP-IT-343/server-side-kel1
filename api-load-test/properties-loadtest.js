...
// default kecil sesuai contoh Postman-mu
const TYPES_LIST = (__ENV.TYPES || 'rumah,apartemen,ruko')
  .split(',').map(s => s.trim()).filter(Boolean);

// Harga (hindari out-of-range)
...
// Auth opsional
const AUTH_TYPE  = (__ENV.AUTH_TYPE || 'none').toLowerCase(); // bearer|none
    ...
    default:
      return {
        ...
        thresholds: {
          ...
          ok_rate: ['rate>0.99'],
          // bonus: pastikan tidak ada 500/429
...
export const options = {
  ...
  // tambah timeout agar cepat gagal saat service macet
  // @ts-ignore
...
export function setup() {
  return {
    authHeader: {} // tidak kirim Authorization
  };
}
...
export function testFlow(data) {
  ...
  // 1) DEFINISIKAN DULU PARAMETERNYA
  ...
  const propertyType = pick(TYPES_LIST); // contoh: rumah|apartemen|ruko (lowercase)
  ...
  if (USE_FORM) {
    // 2) MODE GET + BODY x-www-form-urlencoded (meniru Postman-mu)
    const bodyForm =
  ...
  if (!ok) {
    ...
    if (String(__ENV.LOG_FAIL || 'false').toLowerCase() === 'true') {
      console.error(`FAIL ${res.status} ${url} dur=${res.timings.duration}ms bodyLen=${(res.body || '').length}`);
    }
  }
  okRate.add(ok);
