 ...
 * Following Spring Boot testing best practices with Mockito
 */
...
class AuthServiceTest {
    ...
    void shouldLoginUserSuccessfully() {
        ...
        when(userService.findByUsernameOrEmail(loginRequest.getIdentifier())).thenReturn(Optional.of(testUser));
        ...
        when(jwtUtil.generateRefreshToken(testUser.getUsername(), testUser.getId(), testUser.getRole().getName()))
                .thenReturn(refreshToken);
        when(jwtUtil.generateAccessToken(testUser.getUsername(), testUser.getId(), testUser.getRole().getName()))
        ...
        verify(jwtUtil).generateRefreshToken(testUser.getUsername(), testUser.getId(), testUser.getRole().getName());
        verify(jwtUtil).generateAccessToken(testUser.getUsername(), testUser.getId(), testUser.getRole().getName());
    ...
    void shouldThrowExceptionWhenUserNotFound() {
        // Given
        when(userService.findByUsernameOrEmail(loginRequest.getIdentifier())).thenReturn(Optional.empty());
    ...
    void shouldLogoutUserSuccessfully() {
        ...
        when(userSessionRepository.findActiveByRefreshToken(hashedToken)).thenReturn(Optional.of(testSession));
    ...
    /**
     * Helper method to calculate SHA-256 hash of a token
     * This matches the hashToken method in AuthService
     */
