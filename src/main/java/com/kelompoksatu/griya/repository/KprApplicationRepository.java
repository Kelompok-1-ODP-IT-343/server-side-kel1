...
public interface KprApplicationRepository extends JpaRepository<KprApplication, Integer> {
     ...
     */
    List<KprApplication> findByUserIdAndStatusOrderByCreatedAtDesc(Integer userId, KprApplication.ApplicationStatus status);

    /**
     * Check if user has any pending applications for the same property
     */
    @Query("SELECT COUNT(k) > 0 FROM KprApplication k WHERE k.userId = :userId AND k.propertyId = :propertyId " +
           "AND k.status IN ('SUBMITTED', 'DOCUMENT_VERIFICATION', 'PROPERTY_APPRAISAL', 'CREDIT_ANALYSIS', 'APPROVAL_PENDING')")
    boolean existsPendingApplicationByUserAndProperty(@Param("userId") Integer userId, @Param("propertyId") Integer propertyId);

    /**
     * Get next sequence number for application number generation
     */
    @Query("SELECT COALESCE(MAX(CAST(SUBSTRING(k.applicationNumber, 10) AS integer)), 0) + 1 " +
     ...
     */
    @Query("SELECT k FROM KprApplication k WHERE k.currentApprovalLevel = :levelId AND k.status = 'APPROVAL_PENDING'")
