...
/**
 * REST Controller for KPR Application operations
 * Handles home loan application submissions with JWT authentication
 */
@RestController
...
public class KprApplicationController {
     ...
     * POST /api/me/kpr-applications
     */
    @PostMapping("/kpr-applications")
    @Operation(
            summary = "Submit KPR Application",
            description = "Submit a new KPR (home loan) application for an authenticated user. " +
    ...
    public ResponseEntity<ApiResponse<KprApplicationResponse>> submitKprApplication(
            ...
            @Parameter(description = "KPR application request data", required = true)
            @Valid @RequestBody KprApplicationRequest request,
        ...
        try {
            ...
            if (userId == null) {
                ...
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(ApiResponse.error(
                                "User ID tidak ditemukan dalam token",
                                httpRequest.getRequestURI()
                        ));
            }
            ...
            log.info("KPR application submitted successfully with ID: {} for user: {}",
                    response.getApplicationId(), userId);

            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.success(
                            response,
                            "Aplikasi KPR berhasil disubmit",
                            httpRequest.getRequestURI()
                    ));
    ...
    private HttpStatus determineErrorStatus(String errorMessage) {
        ...
        } else if (lowerMessage.contains("pending application") || lowerMessage.contains("sudah ada aplikasi")) {
